...
//import plugins.utils.UTCfg;
//import plugins.utils.UTStr;
...
public class Node extends DefaultMutableTreeNode {
	...
	public boolean eq(Node node) {
		if (getMethodDeclaration() != null
				&& node.getMethodDeclaration() != null
				&& //
				node.getEntity().getType().isMethod()
				&& getEntity().getType().isMethod()) {
			...
			String myNode = getMethodDeclaration().getName()
					.getFullyQualifiedName();
			String yourNode = node.getMethodDeclaration().getName()
					.getFullyQualifiedName();
			...
			if (bgn17 == bgn14 && end17 == end14 && myNode.equals(yourNode)
					&& myClassName.equals(yourClassName)) {
				return true;
			} else {
				return false;
			}
		...
		if (this.getValue().equals(node.getValue()) && //
				...
				this.getEntity().getSourceRange()
						.equals(node.getEntity().getSourceRange())) {
			return true;
		}
		return false;
	}
	...
	public String getValueReplacedParmWithToken() {
		String replaced = getValue().replaceAll(UTCfg.PARM_VAR_NAME,
				PARM_VAR_TOKEN);
		return replaced;
	}
	...
	public void setSimilarityThreshold(int similar) {
		mSimilarityThreshold = similar;
	}
	...
	public boolean isSameSizeOfParameter(Node aNode) {
		...
		if (myNewValue == null || urNewValue == null
				|| urValueArgList.size() != myValueArgList.size()) {
			return false;
		}
		return true;
	}
	...
	public void printNewValue() {
		...
		while (e.hasMoreElements()) {
			Node iNode = (Node) e.nextElement();
			System.out.println(UTStr.getIndent(iNode.getLevel())
					+ iNode.toNewString());
		}
	}
	 ...
	 * Prints the.
	 * 
	 * @param aIncludedByUser
	 *            the a included by user
	 */
	...
	public boolean isVarDecl() {
		return this.getEntity().getType()
				.equals(JavaEntityType.VARIABLE_DECLARATION_STATEMENT);
	}
	...
	public boolean isForEachStmt() {
		return this.getEntity().getType()
				.equals(JavaEntityType.FOREACH_STATEMENT);
	}
	...
	public boolean isSkip() {
		if (getLevel() == 1) {
			if (((Node) getParent()) != null
					&& ((Node) getParent()).getEntity().getType().isMethod()) {
				if (getEntity().getType().equals(JavaEntityType.MODIFIERS)
						|| //
						getEntity().getType()
								.equals(JavaEntityType.SINGLE_TYPE)
						|| //
						getEntity().getType().equals(
								JavaEntityType.TYPE_PARAMETERS)
						|| //
			...
			if (((Node) o3).getEntity().getType().isMethod() && //
					...
					getEntity().getType().equals(
							JavaEntityType.PARAMETERIZED_TYPE)) {
				return true;
			}
		}
		return false;
	 ...
	 * @param aSrc
	 */
	public void setNewValue_localValDecl(SimpleName aNewName,
			SimpleName aOldName, String aSrc) {
	...
	public void setNewValue(SimpleName aNewName, SimpleName aOldName,
			String aSrc) {
		String oldName = aOldName.getFullyQualifiedName();
		String newName = aNewName.getFullyQualifiedName();
		int bgn14 = aOldName.getStartPosition();
	 ...
	 */
	@SuppressWarnings("unused")
	public void setNewValueForEachStmt(SimpleName aNewName,
			SimpleName aOldName, String aSrc) {
		String oldName = aOldName.getFullyQualifiedName();
		String newName = aNewName.getFullyQualifiedName();
		int bgn14 = aOldName.getStartPosition();
	 ...
	 */
	@SuppressWarnings("unused")
	public void setNewValueForStmt(SimpleName aNewName, SimpleName aOldName,
			String aSrc) {
		String oldName = aOldName.getFullyQualifiedName();
		String newName = aNewName.getFullyQualifiedName();
		int bgn14 = aOldName.getStartPosition();
	 ...
	 */
	@SuppressWarnings("unused")
	public void setNewValueIfStmt(SimpleName aNewName, SimpleName aOldName,
			String aSrc) {
		String oldName = aOldName.getFullyQualifiedName();
		String newName = aNewName.getFullyQualifiedName();
		int bgn14 = aOldName.getStartPosition();
